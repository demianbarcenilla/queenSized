...
function unlockableSkills()
{
	var _ratLeak, _cookieLeak, _ultraRare, _cookieBox, _hpFreeze, _phishingRod, _craft;
	ini_open("unlocks.ini");
		...
		_craft = ini_read_real("unlocks", "33", false);
	ini_close();
	...
	if(_craft)
	{
		arr_poolTable[stage.tunnel, array_length(arr_poolTable[stage.tunnel])] = normal.craft;
		arr_poolTable[stage.ratlantis, array_length(arr_poolTable[stage.ruins])] = normal.craft;
		arr_poolTable[stage.cloud, array_length(arr_poolTable[stage.cloud])] = normal.craft;
	};
}